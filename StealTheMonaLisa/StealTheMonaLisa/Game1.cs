...
namespace StealTheMonaLisa
{
    ...
    public class Game1 : Game
    {
        ...
        // World Map Stuff
        bool growth = false;
        int deleteMissionTimer = 3000;
        int additionMissionTimer = 5000;
        int deleteMis = 99;
        int misGen = 0;
        int misNum;
        int misQuantity = 9;
                ...
                case GameState.WorldMapMenu:
                    {
                        ...
                        for (int i = 0; i < mis.Length; i++)
                        {
                            ...
                            // This right here executes twice for some reason
                            // then when i remove exactly 4 mission form the array
                            // the map starts glitching
                            // GG
                            if (deleteMissionTimer == 0)
                            {
                                deleteMis = rng.Next(0, misQuantity);
                            }
                            if(additionMissionTimer == 0)
                            {
                                AddMission();
                                additionMissionTimer = 5000;
                            }
                            // This code ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ look at it next time
                            //
                            // also finish the add a mission method, then you work on putting all of this together
                            // making missions disappear and reappear randomly!!!
                            // After focus on making more items to put in the mission array
                            ...
                            {                               
                                ...
                                if (growth)
                                {
                                    if (i == deleteMis)
                                    {      
                                            if (mis[deleteMis].Rec.Height == 0 && mis[deleteMis].Rec.Width == 0)
                                            {
                                                deleteMissionTimer = 3000;
                                                RemoveMission(deleteMis);
                                                deleteMis = 99;
                                            }
                                            else
                                            {
                                    ...
                                    {
                                        if (mis[i].Rec.Height > 15 && mis[i].Rec.Width > 15)
                                        {
                                            mis[i].Rec = new Rectangle(mis[i].Rec.X + 1, mis[i].Rec.Y + 1, mis[i].Rec.Height - 1, mis[i].Rec.Width - 1);
                                ...
                                else if (mis[i].Rec.Height < 25 && mis[i].Rec.Width < 25)
                                {
                                    if (i == deleteMis)
                                    {
                                        if (mis[deleteMis].Rec.Height == 0 && mis[deleteMis].Rec.Width == 0)
                            ...
                            {
                                additionMissionTimer -= 1;
                                deleteMissionTimer -= 1;
                                time -= 1;
                            }
                            ...
                            if (i == colorSwitch)
                            {
                                ...
                                if (i == deleteMis && mis[deleteMis] == null)
                                {
        ...
        public void MissionGeneration()
        {
            if (misGen == 0)
            {
                mis = null;
                mis = new Mission[misQuantity];
        ...
        public void RemoveMission(int misToDel)
        {
            Mission[] holster;
            ...
            {
                holster = new Mission[misQuantity - 1];
        ...
        public void AddMission()
        {
            ...
            Mission[] holster = new Mission[misQuantity];
            ...
            holster[misQuantity - 1] = new Mission(items[lv].ElementAt<Item>(itm));
            holster[misQuantity -1].RandomRec(rec);

            int ran = rng.Next(1, 6);
            holster[misQuantity - 1].MissionItem.RandomDesc(ran);
